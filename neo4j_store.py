...
class Neo4jRoleStore(RoleStore):
    ...
    def _load_roles(self) -> List[Dict[str, Any]]:
        ...
        cypher = """
            ...
            MATCH (role:Role)-[edge:HAS_CORE_SKILL {run_id:run.run_id}]->(skill:NormalizedSkill)
    ...
    def _build_role_entry(record) -> Dict[str, Any]:
        ...
        for item in record.get("skills", []) or []:
            ...
            current = skills_by_name.get(key)
            if current is None or (skill["rank"], -skill["weight"]) < (
                current["rank"], -current["weight"]
            ):
                skills_by_name[key] = skill
        skills = sorted(
            skills_by_name.values(),
            key=lambda item: (item["rank"], -item["weight"], item["name"]),
        )
        return {
