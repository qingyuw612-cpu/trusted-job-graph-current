...
CALL {
    ...
    WHERE raw.domain_label = 'IT'
      AND coalesce(raw.raw_job_title, raw.title, '') <> ''
...
CALL {
    OPTIONAL MATCH (processed:ProcessedJD {status:'COMPLETED'})
    RETURN count(processed) AS processed_jds,
...
RETURN raw.version_id AS jd_id,
       coalesce(raw.domain_role, '') AS canonical_role,
       raw.company_id AS company_id,
       raw.company_name AS company_name,
       coalesce(raw.raw_job_title, raw.title) AS raw_job_title,
       coalesce(raw.normalized_job_title, '') AS normalized_job_title,
       coalesce(raw.crawl_keyword, raw.search_keyword, '') AS crawl_keyword,
       raw.publish_time_raw AS posted_at,
       raw.industry AS industry,
       raw.domain_label AS domain_label,
...
# Demo/在线快速查看只取最近一小段 JD，避免每次都把全局图谱分页搬到
# Python。该模式用于快速观察候选，不替代论文中的全量离线统计。
JD_SAMPLE_QUERY = """
MATCH (raw:RawJDVersion)
WHERE raw.version_id IS NOT NULL
MATCH (:RawJob)-[:CURRENT_VERSION]->(raw)
WHERE raw.domain_label = 'IT'
  AND coalesce(raw.raw_job_title, raw.title, '') <> ''
WITH raw
ORDER BY coalesce(raw.observed_epoch, 0) DESC, raw.version_id DESC
LIMIT $sample_limit
RETURN raw.version_id AS jd_id,
       coalesce(raw.domain_role, '') AS canonical_role,
       raw.company_id AS company_id,
       ...
       raw.content_hash AS template_cluster_id,
       raw.observed_epoch AS observed_epoch
ORDER BY observed_epoch DESC, jd_id DESC
"""
...
RETURN raw.version_id AS version_id,
       ...
       coalesce(raw.raw_archive_uri, '') AS raw_archive_uri
"""
...
HISTORICAL_TITLE_QUERY = """
MATCH (raw:RawJDVersion)
WHERE raw.domain_label = 'IT'
  AND coalesce(raw.raw_job_title, raw.title, '') <> ''
WITH raw,
     split(
         replace(split(trim(raw.publish_time_raw), ' ')[0], '-', '/'),
         '/'
     ) AS date_parts
WITH raw,
     CASE WHEN size(date_parts) = 3
          ...
          ELSE null
     END AS posted_date
WHERE posted_date IS NOT NULL AND posted_date < date($cutoff_date)
RETURN coalesce(raw.raw_job_title, raw.title) AS raw_job_title,
       coalesce(raw.normalized_job_title, '') AS normalized_job_title,
       coalesce(raw.domain_role, '') AS historical_domain_role
"""
...
def _safe_source_info(row: dict[str, Any]) -> dict[str, Any]:
    info = {
        ...
        "label": "Neo4j 全局岗位图谱",
        "connected": True,
        # Name discovery is valid from the raw current-version layer.  The
        # active normalization run is only needed by the independent skill
        # change branch and must not gate title discovery.
        "schema_ready": int(row.get("usable_jds") or 0) > 0,
        ...
        "processed_jds": int(row.get("processed_jds") or 0),
...
class Neo4jEvolutionSource:
    ...
    def load_jd_rows(self, *, sample_limit: int = 0) -> list[dict[str, Any]]:
        ...
        expected = max(min(expected_count, sample_limit) if sampled else expected_count, 1)
        ...
        while True:
                ...
                rows.append(
                    {
                        ...
                        "normalized_job_title": str(
                            raw.get("normalized_job_title") or ""
                        ),
                        "crawl_keyword": str(raw.get("crawl_keyword") or ""),
            ...
            if len(page) < self.batch_size:
                break
        if not sampled and len(rows) != expected_count:
            raise Neo4jSourceChangedError(
                "Neo4j 可用 JD 数与分页读取结果不一致；"
                "本轮结果已丢弃，请重新运行。"
            )
        self.captured_info["sample_limit"] = sample_limit
        self.captured_info["sampled_jds"] = len(rows)
        self.captured_info["sample_strategy"] = (
            "latest_observed_epoch" if sampled else "full_paged_read"
        )
        return rows
    ...
    def load_descriptions(
        ...
        *,
        ...
        for index in range(0, len(identifiers), max(1, chunk_size)):
            ...
            # Raw JD bodies are intentionally moved out of Neo4j after
            # ingestion.  Restore them from the append-only archive so role
            # discovery still receives the real recruitment evidence.
            hydrated_rows = hydrate_raw_rows(rows)
            for row in hydrated_rows:
                jd_id = str(row.get("version_id") or "")
                if jd_id:
    ...
    def load_known_titles(self, *, before: datetime | None = None) -> set[str]:
        if not self.captured_info:
            self.capture()
        rows = self.client.query(KNOWN_TITLES_QUERY, access_mode="Read")
        titles: set[str] = set()
        for row in rows:
            values = [
                row.get("role_name"),
        ...
        if before is not None:
            for row in self.client.query(
                ...
                values = (
                    ...
                    row.get("historical_domain_role"),
                )
                titles.update(
                    str(value).strip()
                    for value in values
