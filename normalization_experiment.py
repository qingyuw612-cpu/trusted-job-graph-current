...
class HashingTextEmbedder:
    """Deterministic local fallback used when the optional BGE runtime is unavailable."""
...
class NormalizationExperiment:
    ...
    def _ann_candidate_pairs(self, indices: list[int], phrase_vectors):
        ...
        try:
            ...
            # Chunk the exact NumPy search so a production candidate pool does
            # not allocate an O(n^2) similarity matrix when FAISS is absent.
            neighbors = np.empty((len(indices), neighbor_count), dtype="int64")
            for start in range(0, len(indices), 256):
                stop = min(start + 256, len(indices))
                similarities = vectors[start:stop] @ vectors.T
                if neighbor_count < len(indices):
                    candidates = np.argpartition(
                        -similarities, neighbor_count - 1, axis=1
                    )[:, :neighbor_count]
                    candidate_scores = np.take_along_axis(similarities, candidates, axis=1)
                    order = np.argsort(-candidate_scores, axis=1)
                    neighbors[start:stop] = np.take_along_axis(candidates, order, axis=1)
                else:
                    neighbors[start:stop] = np.argsort(-similarities, axis=1)
