...
WORKBENCH_PAGE = FUSED_FRONTEND_PATH
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="只读检查 Neo4j、工作台页面和讯飞 Spark Lite 凭据状态后退出",
    )
    return parser
...
def _check(neo4j_config_path: Path, data_root: Path) -> dict:
    ...
    # Missing Spark Lite credentials only disables optional semantic review. Neo4j
