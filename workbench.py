...
from .engine import ALGORITHM_VERSION, RESULT_SCHEMA_VERSION, EvolutionEngine
...
SPARK_API_KEY_ENV = "IFLYTEK_SPARK_API_PASSWORD"
LEGACY_MAAS_API_KEY_ENV = "IFLYTEK_MAAS_API_KEY"
# Kept as an import-compatible alias for the existing app health endpoint.
MAAS_API_KEY_ENV = SPARK_API_KEY_ENV
TASK_ID_PATTERN = re.compile(r"^task_[0-9a-f]{32}$")
PROJECT_ROOT = Path(__file__).resolve().parents[1]
FUSED_FRONTEND_PATH = next(
    (
        candidate
        for candidate in (
            PROJECT_ROOT.parent / "qianduan" / "html-main2" / "emerging-roles.html",
            PROJECT_ROOT / "qianduan" / "html-main2" / "emerging-roles.html",
        )
        if candidate.is_file()
    ),
    PROJECT_ROOT / "qianduan" / "html-main2" / "emerging-roles.html",
)
DEFAULT_PAGE_PATH = FUSED_FRONTEND_PATH
ABILITY_PAGE_PATH = Path(__file__).resolve().parent / "static" / "ability_changes.html"
RUN_PARAMETER_NAMES = {
    "cutoff",
    "as_of",
    "baseline_days",
...
def _load_maas_api_key() -> str:
    """Load the Spark Lite credential without exposing it over HTTP."""
    for name in (SPARK_API_KEY_ENV, LEGACY_MAAS_API_KEY_ENV):
...
def check_workbench(
    ...
    return {
        ...
        "page": page_status,
        "spark_lite_configured": bool(_load_maas_api_key()),
    }
...
class EvolutionWorkbenchService:
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "release_id": os.getenv("TG_RELEASE_ID", "development"),
            "new_role_algorithm_version": ALGORITHM_VERSION,
            "new_role_result_schema_version": RESULT_SCHEMA_VERSION,
            "data_mode": "production" if source.get("connected") else "unavailable",
            "source": source,
            "source_error": source_error,
            "spark_lite_configured": bool(_load_maas_api_key()),
            "worker_count": 1,
    ...
    def get_latest_public_run(self) -> dict[str, Any]:
        """Return the newest evidence-complete public new-role snapshot.

        Small incremental windows remain in the audit run list and may update
        the independent ability snapshot immediately.  They do not replace a
        fuller public role page until enough mechanically published candidates
        are present.
        """
        with self._lock:
            completed = sorted(
                (
                    ...
                    for task in self._tasks.values()
                    if task.get("status")
                    in {"REVIEW_READY", "DEGRADED_REVIEW_READY"}
                ),
                key=lambda item: str(
        ...
        latest_task_id = str(completed[0].get("task_id") or "") if completed else ""
        fallback: tuple[dict[str, Any], int] | None = None
        for task in completed:
            try:
                candidates = _read_json(
                    self._task_output_dir(task)
                    / RESULT_ARTIFACTS["new_role_candidates"]
                )
            except (WorkbenchError, OSError, ValueError, json.JSONDecodeError):
                continue
            ...
            public_count = sum(
                ...
                and candidate.get("publication_state") == "PUBLISHED_CANDIDATE"
            )
            if public_count and fallback is None:
                fallback = (task, public_count)
            if public_count >= PUBLIC_SNAPSHOT_MINIMUM:
                return self._public_snapshot_payload(
                    task,
                    public_count,
                    latest_task_id,
                    "LATEST_EVIDENCE_COMPLETE_PUBLIC_SNAPSHOT",
                )
        if fallback is not None:
            task, public_count = fallback
            return self._public_snapshot_payload(
                task,
                public_count,
                latest_task_id,
                "LATEST_NON_EMPTY_PUBLIC_SNAPSHOT",
            )
        return {
            "status": "empty",
            "source_policy": "LATEST_EVIDENCE_COMPLETE_PUBLIC_SNAPSHOT",
            "public_snapshot_minimum": PUBLIC_SNAPSHOT_MINIMUM,
    ...
    def _public_snapshot_payload(
        ...
        return {
            ...
            "public_snapshot_minimum": PUBLIC_SNAPSHOT_MINIMUM,
            "public_candidate_count": public_count,
            "latest_task_id": latest_task_id,
            "is_historical_snapshot": bool(
                latest_task_id and task_id != latest_task_id
            ),
            "task": self._public_task(task),
        }

    def get_latest_skill_changes(self) -> dict[str, Any]:
        """Return the newest completed non-empty ability-change snapshot.

        New-role discovery and existing-role ability evolution are independent.
        A title-only validation run may intentionally set ``skill_review_limit``
        to zero, so its empty ability artifact must not erase the last completed
        ability snapshot shown by the ability page.
        """
        with self._lock:
            completed = sorted(
                (
        ...
        for task in completed:
            try:
                ...
                value = _read_json(output_dir / RESULT_ARTIFACTS["role_skill_changes"])
            except (WorkbenchError, OSError, ValueError, json.JSONDecodeError):
                continue
            ...
            roles = {
                ...
                for row in public_value
            }
            return {
                "status": "ready",
                "source_policy": "LATEST_NON_EMPTY_COMPLETED_ABILITY_UPDATE",
                "is_historical_snapshot": bool(latest_task_id and task_id != latest_task_id),
                "latest_task_id": latest_task_id,
                "algorithm_version": summary.get("algorithm_version") or ALGORITHM_VERSION,
                "result_schema_version": summary.get("result_schema_version") or RESULT_SCHEMA_VERSION,
                "ability_result_summary": {
                    "role_count": len(roles),
                    "change_count": len(public_value),
                    "reviewed_role_count": int(summary.get("skill_review_roles") or 0),
                    "review_task_count": int(summary.get("skill_review_tasks") or 0),
                },
                "task": self._public_task(task),
                "role_skill_changes": public_value,
            }
        return {
            "status": "empty",
            "source_policy": "LATEST_NON_EMPTY_COMPLETED_ABILITY_UPDATE",
    ...
    def get_result(self, task_id: str) -> dict[str, Any]:
        ...
        if task.get("status") not in {"REVIEW_READY", "DEGRADED_REVIEW_READY"}:
            raise WorkbenchError(
                HTTPStatus.CONFLICT,
                "result_not_ready",
                "任务尚未生成可查看的结果。",
        ...
        for key, name in RESULT_ARTIFACTS.items():
            ...
            try:
                ...
                if key == "new_role_candidates" and isinstance(value, list):
                    for candidate in value:
                        ...
                        if not analysis.get("core_responsibilities"):
                            ...
                            if responsibilities:
                                ...
                                candidate["semantic_review"] = semantic
                    # The artifact is ordered for the full review queue, not
                    # for the public page.  Always carry the complete
                    # mechanically published set into the bounded API
                    # window; otherwise a public candidate after the first
                    # 50 recall rows silently disappears from the frontend.
                    public_candidates = [
                        candidate
                        for candidate in value
                        if candidate.get("publication_state")
                        == "PUBLISHED_CANDIDATE"
                        or (candidate.get("human_review") or {}).get(
                            "current_state"
                        )
                        == "CONFIRMED_NEW_ROLE"
                    ]
                    ...
                    value = public_candidates + [
                        candidate
                        for candidate in value
                        if str(candidate.get("candidate_id") or "") not in public_ids
                    ]
        ...
        summary = result["task"].get("summary") if isinstance(result["task"].get("summary"), dict) else {}
    ...
    def _validate_run_parameters(
        ...
        window_semantics = (
            "显式 cutoff/as_of 已提供；current_days 仅保留兼容字段，不参与本次窗口计算。"
            if cutoff and as_of
            else "未显式提供 cutoff/as_of；current_days 用于从 as_of 向前计算窗口。"
        )
        sample_limit = self._bounded_integer(
            payload.get("sample_limit", 0),
            "sample_limit",
            0,
            20000,
        )
        ...
        llm_mode = str(payload.get("llm_mode") or "required").lower()
        ...
        spark_lite_available = bool(_load_maas_api_key())
        if llm_mode == "required" and not spark_lite_available:
            raise WorkbenchError(
                HTTPStatus.PRECONDITION_FAILED,
                "spark_lite_not_configured",
                "服务器尚未配置讯飞 Spark Lite API Password。",
            )
        return {
            ...
            "llm_mode": llm_mode,
            "spark_lite_enabled": llm_mode != "off" and spark_lite_available,
        }
    ...
    def _execute_task(
        ...
        try:
            ...
            config = EvolutionConfig(
                ...
                sample_limit=parameters["sample_limit"],
                ...
                llm_enabled=bool(parameters["spark_lite_enabled"]),
                llm_provider="iflytek_spark_openai",
                llm_model="lite",
                llm_base_url="https://spark-api-open.xf-yun.com/v1/chat/completions",
                llm_api_key_env=SPARK_API_KEY_ENV,
                llm_api_password_env=LEGACY_MAAS_API_KEY_ENV,
                # 岗位名称由确定性机械聚类完成，LLM 只审核 Top-K 候选。
                llm_timeout_seconds=(
                    30 if parameters["sample_limit"] > 0 else 60
                ),
                llm_max_requests=(
                    parameters["role_review_limit"] * 2
                    + parameters["skill_review_limit"]
                ),
                llm_role_max_requests=parameters["role_review_limit"] * 2,
                llm_skill_max_requests=parameters["skill_review_limit"],
                llm_max_retries=1,
                llm_max_output_tokens=2200,
                llm_max_evidence_per_candidate=(
                    3 if parameters["sample_limit"] > 0 else 5
                ),
                llm_max_input_characters=(
                    4500 if parameters["sample_limit"] > 0 else 6000
                ),
                dry_run=True,
            )
            ...
            result_state = str(result.get("state") or "REVIEW_READY")
            if result_state not in {"REVIEW_READY", "DEGRADED_REVIEW_READY"}:
            ...
            self._update_task(
                ...
                stage=result_state,
                message=(
                    "分析完成；部分 AI 辅助建议不完整，全部规则候选仍可进入人工审核。"
                    if result_state == "DEGRADED_REVIEW_READY"
                    else "分析完成，候选与 AI 辅助建议已提交人工审核。"
                ),
                completed_at=_now(),
                run_id=str(result.get("run_id") or ""),
...
class EvolutionWorkbenchRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if path in {"/", "/index.html", "/new-roles", "/new-roles.html"}:
                self._redirect_to_fused_frontend()
                return
            if path in {"/ability-changes", "/ability-changes.html"}:
                self._page(ABILITY_PAGE_PATH)
                return
            ...
            if path == "/api/v1/evolution/runs":
                ...
                return
            if path == "/api/v1/evolution/runs/latest-public":
                self._json(self.service.get_latest_public_run())
                return
            if path == "/api/v1/evolution/skill-changes/latest":
                self._json(self.service.get_latest_skill_changes())
                return
    ...
    def _redirect_to_fused_frontend(self) -> None:
        """Keep the API server from serving a second, stale product shell."""
        target = "http://127.0.0.1:8090/emerging-roles.html"
    ...
    def _security_headers(self) -> None:
        # The product shell is served from 8090 while this API listens on 8070.
        # Keep the allow-list local so review actions can be persisted from the
        # visible workbench without opening the API to arbitrary origins.
        allowed = {
            origin.strip()
    ...
    def log_message(self, format: str, *args: Any) -> None:
        # Do not persist request paths or bodies: they may contain user data.
        del format, args
