...
def write_markdown_report(
    ...
    lines: list[str] = [
        "# 岗位演化检测结论报告",
        "",
        "> 本报告严格区分算法候选、讯飞 Spark Lite 语义复核意见和人工最终决策。"
    ...
    lines.extend(
        _table(
            ...
            [
                ...
                ["窗口语义", (
                    "显式 cutoff/as_of；current_days 不参与计算"
                    if summary.get("cutoff") and summary.get("as_of")
                    else "current_days 计算近期窗口"
                )],
                ...
                ["分析状态", summary.get("analysis_status")],
            ],
        )
    )
    ...
    funnel = (quality.get("role_discovery") or {}).get("funnel") or summary.get("role_funnel") or {}
    if funnel:
        lines.extend(
            [
                "### 3.0 新岗位候选漏斗",
        ...
        lines.extend(
            _table(
                ["阶段", "数量"],
                [
                    ["可用 JD", funnel.get("available_jds")],
                    ["当前标题", funnel.get("current_titles")],
                    ["机械概念簇", funnel.get("mechanical_concepts")],
                    ["近期新增/增长", funnel.get("recent_or_growth")],
                    ["数量/企业/模板门槛", funnel.get("volume_company_template_gate")],
                    ["技能/职责门槛", funnel.get("skill_responsibility_gate")],
                    ["来源/持续性门槛", funnel.get("source_persistence_gate")],
                    ["去除既有岗位别名", funnel.get("existing_alias_removed")],
                    ["Top-K", funnel.get("top_k")],
                    ["AI 已参与", funnel.get("ai_participated", funnel.get("ai_passed"))],
                    ["待人工审核", funnel.get("human_review_ready", funnel.get("top_k"))],
                ],
            )
        )
        rejects = (quality.get("role_discovery") or {}).get("rejected_concepts") or []
        if rejects:
            lines.extend(
                [
                    "",
            ...
            counts = Counter(str(row.get("reason") or "UNKNOWN") for row in rejects)
            lines.extend(_table(["机器原因", "数量"], sorted(counts.items())))
    lines.extend(
        _table(
            ["状态", "数量"],
        ...
        for candidate in role_candidates[:max_role_rows]:
            ...
            role_rows.append(
                [
                    ...
                    candidate.get("display_name") or candidate.get("candidate_title"),
    ...
    if reviewed_candidates:
        ...
        lines.extend(["", "本次参数中没有规则护栏或讯飞 Spark Lite 语义复核结果。"])
    ...
    lines.extend(
        [
            ...
            "- 基础规则为：岗位名称聚类后，近期招聘量或企业覆盖高于历史基线，并达到配置门槛。",
            "- 讯飞 Spark Lite 只复核规则筛出的近期新增/增长候选，用于判断新岗位、岗位细分、别名或噪声，不能替代统计规则和人工批准。",
