...
from raw_jd_layer.archive import hydrate_raw_rows  # noqa: E402
...
class SparkFiveDimensionExtractor:
    ...
    def extract(self, document: JobDocument) -> list[SkillCandidate]:
        ...
        if result is None:
            ...
            except Exception as error:  # Lite 单条异常不得阻断整批入图
                message = f"{document.jd_id}: {type(error).__name__}: {error}"
                self.degraded_errors.append(message[:1000])
                print(f"spark_degraded={message[:500]}", flush=True)
                result = {dimension: [] for dimension in ("知识", "技术", "动机", "特质", "自我概念")}
        else:
...
class ProcessingMetrics:
    ...
    llm_degraded: int = 0
    llm_degraded_errors: list[str] = field(default_factory=list)
    ...
    def run(self, limit: int = 0) -> ProcessingMetrics:
        ...
        while not limit or metrics.rows_read < limit:
            ...
            if not rows:
                break
            output_rows = []
            hydrated = hydrate_raw_rows(item.get("raw") or {} for item in rows)
            for item, raw in zip(rows, hydrated):
                item = {**item, "raw": raw}
                cursor = str(raw.get("version_id") or cursor)
                metrics.rows_read += 1
                degraded_before = (
                    len(self.llm_extractor.degraded_errors)
                    if isinstance(self.llm_extractor, SparkFiveDimensionExtractor)
                    else 0
                )
                try:
                ...
                if isinstance(self.llm_extractor, SparkFiveDimensionExtractor):
                    degraded_errors = self.llm_extractor.degraded_errors[degraded_before:]
                    metrics.llm_degraded += len(degraded_errors)
                    metrics.llm_degraded_errors.extend(degraded_errors)
...
def export_llm_queue(
    ...
    with output_path.open("w", encoding="utf-8") as stream:
        while True:
            ...
            if not rows:
                break
            hydrated = hydrate_raw_rows(row.get("raw") or row for row in rows)
            for raw in hydrated:
                output = {
                    "version_id": raw.get("version_id"),
                    "raw_uid": raw.get("raw_uid"),
                    "title": raw.get("title"),
                    "description": raw.get("description"),
                    "tags": raw.get("tags"),
                    "company_name": raw.get("company_name"),
                    "source_platform": raw.get("source_platform"),
                    "source_role": raw.get("declared_role"),
                    "canonical_role": raw.get("domain_role"),
                }
                stream.write(json.dumps(output, ensure_ascii=False) + "\n")
            written += len(rows)
            next_cursor = str(hydrated[-1].get("version_id") or "")
