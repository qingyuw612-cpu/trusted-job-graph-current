...
def skill_stack_lookup() -> dict[str, str]:
    """Return the checked-in canonical skill taxonomy's stack labels."""
    path = PROJECT_ROOT / "trusted_graph_agent" / "skills_registry.json"
    if not path.exists():
        return {}
    payload = json.loads(path.read_text(encoding="utf-8"))
    return {
        normalize_text(str(item.get("canonical_name") or "")): str(item.get("tech_stack") or "")
...
def load_snapshot(database: Path, normalization_dir: Path) -> PublishSnapshot:
    ...
    concepts: list[dict[str, Any]] = []
    stack_by_name = skill_stack_lookup()
        ...
        concepts.append(
            {
                ...
                # Full normalization artifacts from older runs do not carry
                # this optional field; the publisher fills it from the
                # canonical registry and from mapped AbilityCandidates below.
                "tech_stack": row.get("tech_stack") or stack_by_name.get(
    ...
    unclassified_roles = [
        ...
        if normalize_text(role_name) not in lookup
    ]
    if unclassified_roles:
        raise ValueError(
            "存在未纳入 trusted_graph_agent/it_role_taxonomy.json 的岗位，"
            "已拒绝发布，避免再次写入‘扩展岗位（待细分）’："
            + "、".join(unclassified_roles)
        )
    roles: list[dict[str, Any]] = []
    for role_name in role_names:
        if role_name not in role_totals:
            raise ValueError(f"核心技能岗位在 SQLite 中不存在：{role_name}")
        family = lookup[normalize_text(role_name)]
    ...
    def publish(self, snapshot: PublishSnapshot) -> dict[str, Any]:
        ...
        for batch_number, batch in enumerate(chunks(snapshot.roles, self.batch_size), 1):
            self.query(
                ...
                WITH family, role
                OPTIONAL MATCH (previous_family:RoleFamily)-[old:HAS_ROLE]->(role)
                WHERE previous_family.family_id <> family.family_id
                DELETE old
                WITH family, role
        ...
        # Old releases used a silent catch-all family. Once every published
        # role has been reclassified above, remove the obsolete empty shell.
        self.query(
            """
        ...
        # Keep stack metadata on the canonical normalized entity.  A full
        # normalization CSV may predate this field, while the processor has
        # always retained it on AbilityCandidate.  Pick the deterministic
        # most frequent mapped value, without overwriting a value supplied by
