...
然后打开 `http://127.0.0.1:8090/panorama.html`。本地页面固定调用 `http://127.0.0.1:8010/api/...`；生产页面使用同源 `/api/...`，由反向代理转发。
...
页面本地固定调用 `http://127.0.0.1:8000`，生产使用同源 `/api/v1/resume`。项目根目录的 `start_demo.py` 可以统一启动前端与后端。
