...
from raw_jd_layer.archive import RawArchiveWriter, archive_root as default_archive_root  # noqa: E402
...
FIELD_ALIASES = {
    ...
    # Keep the three title layers separate.  In particular, search_keyword is
    # an audit trail for the crawler route, never a job title.
    "raw_job_title": ["原始职位名称", "职位名称", "job_name", "job_title", "title", "position_name"],
    "crawl_keyword": ["搜索关键词", "crawl_keyword", "search_keyword", "keyword"],
    ...
    "domain_label": ["domain_label", "domain", "领域", "数据域"],
}
...
def prepare_row(record: dict[str, Any], context: FileContext, now: str) -> dict[str, Any]:
    ...
    version_properties = {
        "title": values["title"],
        "raw_job_title": values["raw_job_title"] or values["title"],
        "crawl_keyword": values["crawl_keyword"],
        # This is populated when an upstream producer already supplied a
        # cleaned title.  Discovery always recomputes it from raw_job_title so
        # legacy rows and unprocessed rows follow the same deterministic rule.
        "normalized_job_title": values["normalized_job_title"],
        ...
        "domain_label": values["domain_label"] or context.default_domain_label,
...
def file_context(
    ...
    return FileContext(
        ...
        default_domain_label=default_domain_label,
    )
...
WHERE matching.business_content_hash = row.version_props.content_hash
  OR (coalesce(matching.business_content_hash, '') = ''
  AND coalesce(toString(matching.title), '') = coalesce(toString(row.version_props.title), '')
...
CALL (row, existing) {
  ...
  SET existing.collected_at_raw = row.version_props.collected_at_raw,
      existing.raw_job_title = row.version_props.raw_job_title,
      existing.crawl_keyword = row.version_props.crawl_keyword,
      existing.normalized_job_title = row.version_props.normalized_job_title,
      ...
      existing.domain_label = row.version_props.domain_label,
      ...
      existing.raw_archive_uri = row.version_props.raw_archive_uri,
      existing.raw_archive_format = row.version_props.raw_archive_format,
      existing.description_sha256 = row.version_props.description_sha256,
      existing.description_length = row.version_props.description_length,
...
class RawJDImporter:
    def __init__(
        ...
        self.archive_directory = archive_directory.expanduser().resolve() if archive_directory else None

    @staticmethod
    def archive_row(
        row: dict[str, Any], writer: RawArchiveWriter, now: str
    ) -> tuple[dict[str, Any], dict[str, Any]]:
        raw_properties = dict(row["version_props"])
        description = str(raw_properties.get("description") or "")
        graph_row = {**row, "version_props": dict(raw_properties)}
        graph_row["version_props"].pop("description", None)
        graph_row["version_props"].update(
            {
                "raw_archive_uri": writer.uri,
                "raw_archive_format": "jsonl+gzip;v=1",
                "description_sha256": hashlib.sha256(description.encode("utf-8")).hexdigest(),
                "description_length": len(description),
                "raw_archived_at": now,
            }
        )
        return graph_row, {"version_id": row["version_id"], "raw": raw_properties}
    ...
    def import_file(
        ...
        row_count = 0
        archive_writer = (
            RawArchiveWriter(self.archive_directory, run_id, context.source_file_id)
        ...
        for record in iter_records(path):
            ...
            if archive_writer is not None:
                row, archive_payload = self.archive_row(row, archive_writer, now)
            else:
            ...
            if batch and (len(batch) >= self.batch_size or batch_bytes + row_size > self.max_batch_bytes):
                if archive_writer is not None:
                    archive_writer.append_many(archive_payloads)
...
def main() -> None:
    ...
    parser.add_argument("--keep-description-in-neo4j", action="store_true")
    ...
    environment_archive_root = os.environ.get("RAW_JD_ARCHIVE_ROOT", "").strip()
    configured_archive_root = args.archive_root or Path(
        environment_archive_root or config.get("archive_root") or default_archive_root()
    )
    if not configured_archive_root.is_absolute():
        configured_archive_root = PROJECT_ROOT / configured_archive_root
    importer = RawJDImporter(
        ...
        None
        if args.keep_description_in_neo4j or not bool(config.get("archive_enabled", True))
